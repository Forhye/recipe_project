...
[![✔ 레시피 연구소 WBS](https://img.shields.io/badge/✔_레시피_연구소_WBS-6666)](#-레시피-연구소-wbs)
[![📑 기능 명세](https://img.shields.io/badge/📑_기능_명세-315EDF)](#-기능-명세)
...
|                      ![ISTP](https://github.com/user-attachments/assets/7d93de9c-57af-4d96-a9cb-c91bf4a28b6c)                       | ![ENTJ](https://github.com/user-attachments/assets/999c1997-83e3-4020-945e-e8d689dca254) |   ![INTP](https://github.com/user-attachments/assets/e3f24dcc-8a2c-483e-8b75-344403245a11)   |                ![ISTP](https://github.com/user-attachments/assets/7d93de9c-57af-4d96-a9cb-c91bf4a28b6c)                 |
| :---------------------------------------------------------------------------------------------------------------------------------: | :--------------------------------------------------------------------------------------: | :------------------------------------------------------------------------------------------: | :---------------------------------------------------------------------------------------------------------------------: |
|                                                            정성훈(팀장)                                                             |                                          강지혜                                          |                                            박초롱                                            |                                                         홍성원                                                          |
...
gantt
    ...
    section 프로젝트 기획 및 준비
    사용 기술 탐색 및 회의          :active, a1, 2024-09-23, 3d
    프로젝트 아이디어 회의          :a2, 2024-09-23, 3d
    작업 분담 및 GitHub 세팅        :a3, 2024-09-30, 1d
    컨벤션 세팅 회의                :a4, 2024-10-01, 1d

    section 디자인
    메인 디자인(Figma)             :b1, 2024-09-25, 5d
    상세페이지 디자인 변경          :b3, 2024-10-18, 1d

    section 퍼블리싱
    로그인 페이지 퍼블리싱          :c1, 2024-10-02, 9d
    회원가입 페이지 퍼블리싱        :c2, 2024-10-04, 6d
    프로필 페이지 퍼블리싱          :c3, 2024-10-10, 6d
    메인페이지 퍼블리싱             :c4, 2024-10-04, 6d
    레시피 상세페이지 퍼블리싱      :c5, 2024-10-02, 4d

    section 기능 개발
    레시피 상세페이지 기능 추가   :d1, 2024-10-06, 5d
    댓글 CRUD 기능 추가            :d3, 2024-10-11, 4d
    상세페이지 기능 버그 수정       :d6, 2024-10-15, 7d
    좋아요     :d4, 2024-10-18, 1d
    댓글 유저 검증        :d5, 2024-10-19, 1d

    회원가입 기능 추가              :d7, 2024-10-07, 4d
    회원가입 기능 추가(2)           :d7, 2024-10-11, 4d
    프로필 페이지 기능 추가         :d9, 2024-10-15, 5d
    메인 페이지 기능 추가      :d10, 2024-10-15, 6d
    검색                 :d11, 2024-10-16, 1d
    정렬      :d13, 2024-10-20, 1d
    추천 카드        :d14, 2024-10-16, 1d
    404 페이지 추가                :d15, 2024-10-17, 2d
    레시피 AI 페이지 추가           :d16, 2024-10-18, 3d

    section 파일 구조 및 관리
    파일 구조 추가 및 수정          :e1, 2024-10-04, 1d
    Context API 추가               :e2, 2024-10-11, 1d

    section 테스트 및 검토
    전체 페이지 및 기능 검토        :f1, 2024-10-20, 2d
```
...
- **데이터 로드**
  - Firestore에서 레시피 데이터를 읽어와 UI에 반영
  - `recipes` 컬렉션에서 레시피 ID로 데이터 조회
- **댓글 기능**
  - Firestore `comments` 서브컬렉션을 통해 댓글 데이터 CRUD 처리
  - 작성자 검증 및 댓글 삭제 권한 제한 기능 포함

### 2. 레시피 좋아요 기능

- Firestore `hearted` 서브컬렉션에 사용자 정보(email, uid, nickname)를 저장
- 좋아요 클릭 시:
  - 현재 사용자 ID로 데이터 추가
  - `hearts` 필드 카운트 +1 업데이트
- 좋아요 취소 시:
  - 사용자 ID로 데이터 삭제
  - `hearts` 필드 카운트 -1 업데이트
...
- 메인 페이지에서 레시피 제목, 태그를 기준으로 검색 가능
- Enter 키 이벤트로 검색 동작 지원
- 검색어가 없을 경우 전체 리스트 반환

### 4. 정렬 및 필터 기능

- 메인 페이지 또는 검색 리스트 페이지에서 최신순, 좋아요, 조회수 순으로 리스트 정렬 가능
- 1,2,3단계의 요리 레벨로 원하는 레벨의 레시피만 필터 가능

### 4. 회원가입 및 사용자 관리

- Firebase Authentication을 활용한 회원가입 및 로그인 기능
- 사용자 데이터는 Firestore `users` 컬렉션에 저장
- 사용자 정보 수정 가능 (프로필 페이지)
- Firebase를 통해 google 로그인 가능

### 5. 404 페이지

- 잘못된 경로 접근 시 사용자에게 404 페이지 표시
- 홈으로 이동 버튼 제공
...
- README.md 수정
    <ol>
        <li><s>WBS 설명 부족</s> ✔추가완료</li>
        <li><s>기능 명세,</s> ✔추가완료 프로젝트 시각화 부분이 부족해서 추가하기</li>
        <li>폴더구조 추가도 고려하기</li>
    </ol>
...
- hook
    <ol>
        <li><s>usePageStyle.ts의 경우 돔을 바로 컨트롤하므로 훅으로 쓰기에 적합하지 않음</s> ✔수정완료(사용 훅 부분을 지우고 module CSS로 코드 변경/훅 삭제)</li>
        <li>useThumbnailimgUpload.ts 는 에어를 단순히 콘솔로만 로깅을 하고 있어서 유저가 확인하기 어려운 부분이 아쉬움</li>
    </ol>

- 이미지 최적화
    <ol>
        <li><s>현재 이미지 최적화가 되어있지 않음</s> ✔이미지 업로드 최적화 추가 완료</li>
    </ol>
